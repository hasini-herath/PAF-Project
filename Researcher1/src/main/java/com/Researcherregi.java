...
@Produces(MediaType.TEXT_PLAIN)
public String deletereseacher(String resercherData)
{
//Convert the input string to an XML document
 Document doc = Jsoup.parse(resercherData, "", Parser.xmlParser());

//Read the value from the element <itemID>
 String rid = doc.select("rid").text();
 String output = itemObj.deletereseacher(rid);
return output;
}
...
@Produces(MediaType.TEXT_PLAIN)
public String updatereseacher(String resercherData)
{
//Convert the input string to a JSON object
 JsonObject itemObject = new JsonParser().parse(resercherData).getAsJsonObject();
//Read the values from the JSON object
 ...
 String output = itemObj.updatereseacher(rid, fname, lname, phone, bdate,email,inoctgry,country,edustate,password,cpasswored);
return output;
}
}
